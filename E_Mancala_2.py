...
def distribute_balls(N, A, M, B):
    # Initialize array to represent number of balls in each box
    boxes = [0] * N
    for i in range(N):
        boxes[i] = A[i]

    # Initialize array to track number of balls added to each box during each operation
    added_balls = [0] * N

    # Iterate over each operation
    for i in range(M):
        balls_in_hand = boxes[B[i]]
        boxes[B[i]] = 0  # Empty the box

        # Calculate range of boxes affected by this operation
        start_index = (B[i] + 1) % N
        end_index = (B[i] + min(balls_in_hand, N - 1)) % N

        # Increment/decrement added_balls for affected boxes
        added_balls[start_index] += balls_in_hand
        added_balls[end_index] -= balls_in_hand

    # Calculate final count for each box
    current_balls = 0
    for i in range(N):
        current_balls += added_balls[i]
        boxes[i] += current_balls
