...
void solve()
{
    ...
    function<void(int, int, int)> h2 = [&](int i, int x, int y) {
        if (i == n) {
            if (x == 6 && y == 0)
                res[0]++;
            return;
        ...
        if (x == 6 && y == 0)
            return;
        if ((y + 1) < 7 && (y - 1) >= 0 && vis[x][y + 1] == 0 && vis[x][y - 1] == 0 && ((x + 1) > 6 || vis[x + 1][y] == 1) && ((x - 1) < 0 || vis[x - 1][y] == 1))
            return;
        if ((x + 1) < 7 && (x - 1) >= 0 && vis[x + 1][y] == 0 && vis[x - 1][y] == 0 && ((y + 1) > 6 || vis[x][y + 1] == 1) && ((y - 1) < 0 || vis[x][y - 1] == 1))
            return;
        
        if ((x-1<0 || vis[x-1][y]==1) && (x+1>6 || vis[x+1][y]==1))
        {
            if (y-1>=0 && vis[x][y-1]==1){
                if (x-1>=0 and vis[x-1][y-1]==0)return;
                if (x+1<7 and vis[x+1][y-1]==0)return;
            }
            if (y+1<7 && vis[x][y+1]==1){
                if (x-1>=0 and vis[x-1][y+1]==0)return;
                if (x+1<7 and vis[x+1][y+1]==0)return;
            }
        }
        if ((y-1<0 || vis[x][y-1]==1) && (y+1>6 || vis[x][y+1]==1))
        {
            
            if (x-1>=0 && vis[x-1][y]==1){
                if (y-1>=0 and vis[x-1][y-1]==0)return;
                if (y+1<7 and vis[x-1][y+1]==0)return;
            }
            if (x+1<7 && vis[x+1][y]==1){
                if (y-1>=0 and vis[x+1][y-1]==0)return;
                if (y+1<7 and vis[x+1][y+1]==0)return;
            }
        ...
        if (s[i] == '?') {
            for (auto &[_, d] : dir) {
                auto [dx, dy] = d;
                if (0 <= (x + dx) && (x + dx) < 7 && 0 <= (y + dy) && (y + dy) < 7 && vis[x + dx][y + dy] == 0) {
                    vis[x + dx][y + dy] = 1;
                    h2(i + 1, x + dx, y + dy);
                    vis[x + dx][y + dy] = 0;
                }
