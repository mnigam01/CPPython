...
def solve():
    ...
    mini = inf
    maxi = -inf
    myhealth,myattack = lst()
    monsterhealth,monsterattack = lst()
    k,w,a = lst()
    for i in range(k+1):
        mynewhealth,mynewattack = myhealth+i*a,myattack+(k-i)*w
        x,y = ceil(monsterhealth,mynewattack), ceil(mynewhealth,monsterattack)
        z = min(x,y)
        # de(mynewhealth,mynewattack)
        # de(x,y)
        # de(z)
        if monsterhealth-z*mynewattack<=0:
            # de(mynewhealth,mynewattack)
            gh(yes.upper())
            return
