...
def solve():
    ...
    maxi = -inf
    h,w,m = lst()
    operations = matrixNum(m)
    def count_cells(H, W, M, operations):
        row_colors = {i: 0 for i in range(1, H + 1)}  # Initialize row colors
        col_colors = {i: 0 for i in range(1, W + 1)}  # Initialize column colors
        
        # Iterate through each operation
        for op in operations:
            T, A, X = op
            if T == 1:
                row_colors[A] = X  # Update row color
            else:
                col_colors[A] = X  # Update column color
        
        # Count the number of cells for each color
        color_counts = {}
        for row in row_colors.values():
            if row != 0:
                color_counts[row] = color_counts.get(row, 0) + W
        for col in col_colors.values():
            if col != 0:
                color_counts[col] = color_counts.get(col, 0) + H
            
        return len(color_counts), color_counts
    
    x =count_cells(h,w,m, operations)
    de(x)
    # row = [[0,-1] for i in range(h)]
    # col = [[0,-1] for i in range(w)]
    # for _ in range(m):
    #     t,a,x = lst()
    #     a-=1
    #     if t==1:
