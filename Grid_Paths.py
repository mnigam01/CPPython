...
def solve():
    ...
    def h(i=0,x=0,y=0):
        ...
        if x==6 and y==0:
            return 
        if y+1<7 and y-1>=0 and vis[x][y+1]==vis[x][y-1]==0 and (x+1>6 or vis[x+1][y]==1) and (x-1<0 or vis[x-1][y]==1):
            return 
        if x+1<7 and x-1>=0 and vis[x+1][y]==vis[x-1][y]==0 and (y+1>6 or vis[x][y+1]==1) and (y-1<0 or vis[x][y-1]==1):
            return
            
        if s[i]=='?':
            for dx,dy in dir.values():
                if 0<=x+dx<7 and 0<=y+dy<7 and vis[x+dx][y+dy]==0:
                    vis[x+dx][y+dy]=1
                    h(i+1,x+dx,y+dy)
                    vis[x+dx][y+dy]=0
        else:
