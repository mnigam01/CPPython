...
void solve()
{
    ...
    for (int i = 0; i < n; ++i) {
        cin >> a[i][0] >> a[i][1];
        a[i][2] = i; // Index of the item
    }
    
    sort(a.begin(), a.end(), [](const vector<int>& x, const vector<int>& y) {
        return (x[0] < y[0]) || (x[0] == y[0] && x[1] > y[1]);
    });
    vector<int> itcontains(n, 0); // Initialize to 0
    vector<int> beingcontained(n, 0); // Initialize to 0
    ...
    for (auto i:itcontains){
        cout<<i<<" ";
    }
    cout<<endl;
    for (auto i:beingcontained){
        cout<<i<<" ";
    }
    cout<<endl;
